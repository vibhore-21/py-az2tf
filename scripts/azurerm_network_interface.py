    ...
    if crf in tfp:
        ...
        for i in range(0, count):
            ...
            try:
                ...
                fr.write(
                    '\t network_security_group_id = azurerm_network_security_group.' + snsgrg + '__' + snsg + '.id\n')
            except KeyError:
                pass
            ...
            for j in range(0, icount):
                ...
                subname = azr[i]["properties"]["ipConfigurations"][j]["properties"]["subnet"]["id"].split("/")[
                    10].replace(".", "-")
                ...
                # todo: fix subnet id when it is in same rg
                # fr.write('\t\t subnet_id = azurerm_subnet.' + subrg + '__' + subname + '.id\n')
                fr.write('\t\t subnet_id = "{}"\n'.format(azr[i]["properties"]["ipConfigurations"][j]["properties"][
                                                            "subnet"]["id"]))
                ...
                try:
                    pubipnam = \
                        azr[i]["properties"]["ipConfigurations"][j]["properties"]["publicIPAddress"]["id"].split(
                            "/")[8].replace(".", "-")
                    pubiprg = \
                        azr[i]["properties"]["ipConfigurations"][j]["properties"]["publicIPAddress"]["id"].split("/")[
                            4].replace(".", "-").lower()
                ...
                try:
                    ...
                    for k in range(0, kcount):
                        asgnam = \
                            azr[i]["properties"]["ipConfigurations"][j]["properties"]["applicationSecurityGroups"][k][
                                "id"].split("/")[8].replace(".", "-")
                        asgrg = \
                            azr[i]["properties"]["ipConfigurations"][j]["properties"]["applicationSecurityGroups"][k][
                                "id"].split("/")[4].replace(".", "-").lower()
                        if asgrg[0].isdigit(): asgrg = "rg_" + asgrg
                        fr.write(
                            '\t\t application_security_group_ids = ["${azurerm_application_security_group.' + asgrg + '__' + asgnam + '.id}"]\n')
                except KeyError:
                    pass

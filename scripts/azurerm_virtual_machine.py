        ...
        for i in range(0, count):
            ...
            if icount > 0:
                ...
                for j in range(0, icount):
                    vmnetpri = False
                    vmnetid = azr[i]["properties"]["networkProfile"]["networkInterfaces"][j]["id"].split("/")[
                        8].replace(".", "-")
            ...
            if vmtype == "Linux":
                ...
                if vmdispw:
                    try:
                        vmsshpath = azr[i]["properties"]["osProfile"]["linuxConfiguration"]["ssh"]["publicKeys"][0][
                            "path"]
                        vmsshkey = azr[i]["properties"]["osProfile"]["linuxConfiguration"]["ssh"]["publicKeys"][0][
                            "keyData"]
            ...
            try:
                ...
                if vmoscreoption == "Attach":
                    try:
                        vmosmdtyp = azr[i]["properties"]["storageProfile"]["osDisk"]["managedDisk"][
                            "storageAccountType"]
